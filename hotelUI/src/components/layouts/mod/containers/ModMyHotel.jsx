...
import { Form, Input, InputNumber, Button, message, Tag } from "antd";
...
export const ModMyHotel = () => {
    ...
    useEffect(() => {
        if (storeHotel) {
            setHotelView((prev) => (prev && prev.id === storeHotel.id ? { ...prev, ...storeHotel } : storeHotel));
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            ...
            const res = await fetch(`${HOTEL_API_BASE}/edit/${hotelView.id}`, {
                method: "PUT",
                headers,
                body: JSON.stringify({ ...hotelView, ...values }),
            });
        ...
        <div className="flex flex-col lg:flex-row gap-4 p-4 items-stretch">
            ...
            <div className="lg:w-3/5">
                <div className="rounded-2xl themed-card shadow h-full flex flex-col overflow-hidden">
                    {/* Ảnh */}
                    <div className="relative w-full h-[240px] sm:h-[280px] md:h-[330px] bg-black/5">
                        <img
                    ...
                    <div className="p-4 pt-3 flex flex-col space-y-3">
                        <div>
                            <h2 className="flex items-center gap-3">
                                ...
                                <span
                                    ...
                                    style={{ color: "var(--text)", textShadow: "0 1px 2px rgba(0,0,0,0.22)" }}
                                >
                                    {hotelView.name}
                                </span>
                            </h2>
                        ...
                        <div className="space-y-2 text-[14px] sm:text-[15px]">
                            <div className="flex">
                                <span className="w-24 sm:w-28 font-semibold">Phone:</span>
                                <span className="flex-1">
                                    ...
                                    >
                                        ...
                                        <span>{copiedPhone ? "Copied!" : hotelView.phone || "Not set"}</span>
                                    </span>
                                </span>
                            </div>

                            <div className="flex">
                                <span className="w-24 sm:w-28 font-semibold">Rating:</span>
                                <span className="flex-1">
                                    <span
                                        className={reviewsUrl ? clickableRowClass : "inline-flex items-center gap-2"}
                                        role={reviewsUrl ? "button" : undefined}
                                        tabIndex={reviewsUrl ? 0 : -1}
                                        title={reviewsUrl ? "See Google reviews" : ""}
                                        onClick={() => reviewsUrl && window.open(reviewsUrl, "_blank", "noopener")}
                                        onKeyDown={(e) => {
                            ...
                            <div className="flex">
                                <span className="w-24 sm:w-28 font-semibold">Amenities:</span>
                                <span className="flex-1">{hotelView.amenities || "Not set"}</span>
                            </div>
                        </div>
            ...
            <div className="lg:w-2/5">
                <div className="rounded-2xl themed-card shadow p-4 h-full flex flex-col">
                    <div className="flex items-center gap-4 mb-4">
                        ...
                        <h2 className="text-xl font-extrabold leading-none" style={{ color: "var(--text)" }}>
                            Configure My Hotel
                        </h2>
                    </div>
                    ...
                    >
                        ...
                        <div className="mt-auto pt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-t border-white/40">
                            <span className="text-[11px]" style={{ color: "var(--muted)" }}>
                                Changes will apply to both public page and dashboard stats.
                            </span>
                            <Button type="primary" htmlType="submit" loading={saving} className="px-4" size="small">
                                Save changes
                            </Button>
                        </div>
                    </Form>
