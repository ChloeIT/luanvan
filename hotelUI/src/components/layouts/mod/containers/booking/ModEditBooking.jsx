// src/components/layouts/mod/containers/booking/ModEditBooking.jsx
...
export const ModEditBooking = ({
    isModalEditVisible,
    setIsModalEditVisible,
    itemACtion,   // booking được chọn
    onUpdated,    // ModBookings truyền fetchBookings vào
}) => {
    const { rooms } = useSelector((state) => state.room);

    const [hotelId, setHotelId] = useState(null); // chỉ để hiển thị
    ...
    const handleOk = async () => {
        ...
        try {
            ...
            console.error("Error updating booking (MOD):", err);
    ...
    return (
        <Modal
            ...
            width={540}
        >
            {/* Hotel (hiển thị, không đổi) */}
            <div className="mb-4">
                <label className="block font-medium mb-1">Hotel</label>
                <Select value={hotelFromBooking?.id} className="w-full" disabled>
                    {hotelFromBooking && (
            ...
            <div className="mb-2">
                ...
                <Select value={payment} onChange={setPayment} className="w-full">
