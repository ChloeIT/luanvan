...
export const CompareButton = () => {
  ...
  const hotelNameById = useMemo(
    () => new Map(hotels.map((h) => [String(h.id), h.name])),
    [hotels]
  );
  ...
  // ✅ EXCLUSIVE: widget khác mở -> compare tự đóng
  useEffect(() => {
    const onExclusive = (e) => {
      if (e?.detail?.name !== "compare") setIsOpen(false);
    };
    window.addEventListener("ui:exclusive-open", onExclusive);
    return () => window.removeEventListener("ui:exclusive-open", onExclusive);
  }, []);

  // 5) Khi hotels load/thay đổi: upgrade & ghi "im lặng"
  useEffect(() => {
  ...
  const open = () => {
    setCompares(upgrade(read()));
    window.dispatchEvent(
      new CustomEvent("ui:exclusive-open", { detail: { name: "compare" } })
    );
    setIsOpen(true);
  };
        ...
        {compares.length === 0 ? (
          ...
          <div className="compare-grid compare-grid--4">
            {compares.map((room) => {
              const byId =
                hotelNameById.get(
                  String(room.hotel_id ?? room.hotelId ?? room?.hotel?.id ?? "")
                ) ||
              ...
              return (
                <RoomCard
                  ...
                  hotelId={room.hotel_id ?? room.hotelId ?? room?.hotel?.id}
